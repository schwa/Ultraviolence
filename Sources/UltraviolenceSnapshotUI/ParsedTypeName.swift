...
struct ParsedTypeName {
    ...
    let genericParameters: [ParsedTypeName]?
    ...
    private static func extractGenerics(from string: String) -> (base: String, generics: [ParsedTypeName]?) {
    ...
    private static func parseGenericParameters(_ content: String) -> [ParsedTypeName] {
        var parameters: [ParsedTypeName] = []
        var currentParam = ""
        var depth = 0
        ...
        for char in content {
            if char == "<" {
                ...
                if !trimmed.isEmpty {
                    parameters.append(ParsedTypeName(trimmed))
                }
                currentParam = ""
            } else {
                currentParam.append(char)
    ...
    var nameWithGenerics: String {
        if let generics = genericParameters {
            let genericString = generics.map { $0.nameWithGenerics }.joined(separator: ", ")
    ...
    var fullName: String {
        ...
        if let generics = genericParameters {
            let genericString = generics.map { $0.fullName }.joined(separator: ", ")
