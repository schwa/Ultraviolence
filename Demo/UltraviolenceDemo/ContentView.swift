...
struct TeapotRenderPass: RenderPass {
    ...
    [[fragment]] float4 fragment_main(
        ...
        float rimIntensity = 0.5 * rim;  // Adjust the intensity of the rim light as needed
    ...
    var body: some View {
        TimelineView(.animation) { timeline in
            ...
            .onChange(of: timeline.date, initial: true) {
                angle = Angle(degrees: (timeline.date.timeIntervalSince1970 * 400).truncatingRemainder(dividingBy: 360))
            }
        }
