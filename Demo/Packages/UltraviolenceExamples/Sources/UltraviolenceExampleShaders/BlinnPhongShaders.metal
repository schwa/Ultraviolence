...
[[vertex]]
Fragment vertex_main(uint instance_id [[instance_id]], Vertex in [[stage_in]],
...
fragment_main(Fragment in [[stage_in]],
  ...
  if (material[instance_id].ambientSource == kColorSourceColor) {
    ...
    ambientColor =
        material[instance_id]
            .ambientTexture
            .sample(material[instance_id].ambientSampler, in.textureCoordinate)
            .rgb;
  }
  ...
  if (material[instance_id].diffuseSource == kColorSourceColor) {
    ...
    diffuseColor =
        material[instance_id]
            .diffuseTexture
            .sample(material[instance_id].diffuseSampler, in.textureCoordinate)
            .rgb;
  }
  ...
  if (material[instance_id].specularSource == kColorSourceColor) {
    ...
    specularColor =
        material[instance_id]
            .specularTexture
            .sample(material[instance_id].specularSampler, in.textureCoordinate)
            .rgb;
  }
...
///
/// This function calculates the color contribution from ambient, diffuse, and
/// specular lighting for a surface based on the Blinn-Phong or Phong reflection
/// model. It supports multiple light sources, applies attenuation based on
/// distance, and determines specular highlights based on the selected shading
/// model.
///
/// - Parameters:
...
float3 CalculateBlinnPhong(
    ...
    const float3 normal,
    constant BlinnPhongLightingModelArgumentBuffer &lightingModel,
    const float shininess, const float3 ambientColor, const float3 diffuseColor,
  ...
  // Compute Lambertian reflection (diffuse lighting). The dot product measures
  // how aligned the normal is with the light direction. If the normal faces
  // away from the light, the dot product will be negative, so we clamp it to 0.
  ...
  for (int index = 0; index < lightingModel.lightCount; ++index) {
    ...
    // Compute Lambertian reflection (diffuse lighting). The dot product
    // measures how aligned the normal is with the light direction. If the
    // normal faces away from the light, the dot product will be negative, so we
    // clamp it to 0.
    const float lambertian = max(dot(lightDirection, normal), 0.0);
    if (lambertian == 0.0) {
      continue; // No contribution from this light if it does not reach the
                // surface
    }

    // Apply attenuation to simulate how light weakens over distance. This is a
    // standard quadratic falloff model: 1 / (a + b*d + c*d²). The constants
    // 0.09 and 0.032 are chosen to approximate realistic light falloff. Higher
    // values make light fall off more quickly.
    const float attenuation = 1.0 / (1.0 + 0.09 * distanceSquared +
                                     0.032 * distanceSquared * distanceSquared);

    float specular = 0.0;

    if (!phongMode) {
      // Blinn-Phong Specular Reflection: Instead of computing the perfect
      // reflection direction (like Phong), Blinn-Phong uses the halfway vector
      // (H) between the light direction (L) and the view direction (V). This
      // acts as an "approximate" reflection direction. H = (L + V) / |L + V|.
      // The normal is then compared to this halfway vector to determine the
      ...
      // Phong Specular Reflection: This method explicitly calculates the
      // reflection of the light direction (L) across the normal (N) and
      // compares it with the view direction (V). R = 2 * (N · L) * N - L. The
      // specular intensity is based on how well the reflected vector aligns
      // with V. https://en.wikipedia.org/wiki/Phong_reflection_model
      const float3 reflectionDirection = reflect(-lightDirection, normal);
      specular =
          pow(max(dot(reflectionDirection, viewDirection), 0.0), shininess);
    }
