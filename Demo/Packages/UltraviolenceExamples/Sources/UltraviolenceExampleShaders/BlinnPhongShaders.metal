...
namespace BlinnPhong {
    ...
    /// point.
    ///
    /// This function calculates the color contribution from ambient, diffuse,
    /// and specular lighting for a surface based on the Blinn-Phong or Phong
    /// reflection model. It supports multiple light sources, applies
    /// attenuation based on distance, and determines specular highlights based
    /// on the selected shading model.
    ///
    /// - Parameters:
    ...
    float3 CalculateBlinnPhong(
        ...
        // normal faces away from the light, the dot product will be negative,
        // so we clamp it to 0.
        ...
        for (int index = 0; index < lightingModel.lightCount; ++index) {
            ...
            // Compute Lambertian reflection (diffuse lighting). The dot product
            // measures how aligned the normal is with the light direction. If
            // the normal faces away from the light, the dot product will be
            // negative, so we clamp it to 0.
            const float lambertian = max(dot(lightDirection, normal), 0.0);
            if (lambertian == 0.0) {
                continue; // No contribution from this light if it does not
                          // reach the surface
            }

            // Apply attenuation to simulate how light weakens over distance.
            ...
            if (!phongMode) {
                // Blinn-Phong Specular Reflection: Instead of computing the
                // perfect reflection direction (like Phong), Blinn-Phong uses
                // the halfway vector (H) between the light direction (L) and
                // the view direction (V). This acts as an "approximate"
                // reflection direction. H = (L + V) / |L + V|. The normal is
                ...
                // the reflection of the light direction (L) across the normal
                // (N) and compares it with the view direction (V). R = 2 * (N ·
