...
fragment float4 fragment_main(VertexOutput in [[stage_in]],
  ...
  //                                      float2 gridLineWidth = float2(0.005,
  //                                      0.995); // Line width control float
  //                                      gridVal = pristineGrid(in.uv,
  //                                      gridLineWidth); return float4(gridVal,
  //                                      gridVal, gridVal, 1.0); // Grayscale
  //                                      grid output
}
...
float pristineGrid(float2 uv, float2 lineWidth) {
  ...
  gridSmooth =
      mix(gridSmooth, targetWidth, saturate(uvLengthDerivatives * 2.0 - 1.0));
