...
namespace LambertianShader {
    ...
    VertexOut lambertian(float3 position, float3 normal,
                         constant float4x4 &projectionMatrix,
                         float4x4 modelMatrix, constant float4x4 &viewMatrix,
                         float3 color) {
        VertexOut out;
        ...
        // in.
        out.position =
            projectionMatrix * viewMatrix * modelMatrix * objectSpace;
        out.worldPosition = (modelMatrix * objectSpace).xyz;
        float3x3 normalMatrix = float3x3(modelMatrix[0].xyz, modelMatrix[1].xyz,
                                         modelMatrix[2].xyz);
    ...
    [[vertex]] VertexOut
    vertex_main(const VertexIn in [[stage_in]],
                constant float4x4 &projectionMatrix [[buffer(1)]],
                constant float4x4 &modelMatrix [[buffer(2)]],
                constant float4x4 &viewMatrix [[buffer(3)]],
                constant float3 &color [[buffer(4)]]) {
        return lambertian(in.position, in.normal, projectionMatrix, modelMatrix,
                          viewMatrix, color);
    }

    [[vertex]] VertexOut
    vertex_instanced(const VertexIn in [[stage_in]],
                     constant float4x4 &projectionMatrix [[buffer(1)]],
                     constant float4x4 &viewMatrix [[buffer(3)]],
    ...
    [[fragment]] float4 fragment_main(VertexOut in [[stage_in]],
                                      constant float3 &lightDirection
                                      [[buffer(1)]],
        ...
        float rimIntensity =
            0.25 * rim; // Adjust the intensity of the rim light as needed
