...
VertexOut lambertian(float3 position, float3 normal,
                     constant float4x4 &projectionMatrix, float4x4 modelMatrix,
                     constant float4x4 &viewMatrix, float3 color) {
  VertexOut out;
  ...
  float3x3 normalMatrix =
      float3x3(modelMatrix[0].xyz, modelMatrix[1].xyz, modelMatrix[2].xyz);
